...
- **Sign-up gifts (δώρα) are real but volatile and conditional.** Vouchers / bill credits (e.g. a €50 Πλαίσιο voucher, monthly referral credits) materially cut the *first-year* cost — count them in the first-year total **only**, never as a recurring rate, and record the condition (e-contract, "not claimed in the last 6 months"). Track per offer as `gift_eur` (+ `gift_conditions`); exclude from steady-state.
- **How a floating price is set varies (`pricing_basis`).** Same color can differ: `green-monthly` (announced on the 1st, known for that month) vs `index-prev-month` (ΤΕΑ of the previous month) vs `dynamic-hourly`. The more it's announced ahead, the more predictable — note it; don't treat all "floating" as equally knowable.
- **Per-offer behaviour caveats matter.** Some suppliers claw back the consistency discount on the final-bill consumption, change discounts improperly, or attach an exit fee per remaining month. Capture a `caveat` and `exit_fee_per_month` rather than a vague "terms apply".
